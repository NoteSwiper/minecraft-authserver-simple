...
message_servername = "Authentication server"
message_authenticated = "Issued a authentication card!"

server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket.bind((svhost, svport))
server_socket.listen()
print(f"Listening a authentication request server at {svhost}:{svport}")
...
def handle_client(client, address):
    ...
    if next_state == 1:
        ...
        if res != b"\x00":
            client.close()
            return
        data = json.dumps({"version": {"name": message_servername, "protocol": protocol_version}, "players": {"max": 1, "online": 0}, "description": {"text": message_servername}}).encode()
    ...
    if next_state == 2:
        ...
        print("Created a authentication card for "+has_joined_response["name"]+" ("+has_joined_response["id"]+")")
        data = json.dumps({"text": "§a"+message_authenticated+"§r\n§bUsername:§r §f"+has_joined_response["name"]+"§r\n§bVerification code:§r §f"+verify_code+"§r"}).encode()
        data = b"\x00"+encode_varint(len(data))+data
        data = encode_varint(len(data))+data
        data = encryptor.update(data)
        client.send(data)
